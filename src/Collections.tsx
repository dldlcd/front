...
export default function Collections() {
  ...
  const handleSearch = async () => {
    ...
    // ✅ 검색 로그 저장
    await fetch("https://54.180.117.72/api/searchlog", {
      method: "POST",
      headers: {
  ...
  const handleTagClick = async (tag: string) => {
    ...
    // ✅ 서버에 요청
    const res = await fetch(`https://54.180.117.72/api/outfits?${newParams.toString()}`);
    const data = await res.json();
    setOutfits(data);
  };
  ...
  useEffect(() => {
    const fetchFilteredOutfits = async () => {
      try {
        const res = await fetch(`https://54.180.117.72/api/outfits?${searchParams.toString()}`);
        const data = await res.json();
        setOutfits(data);
      } catch (err) {
  ...
  useEffect(() => {
    fetch('https://54.180.117.72/api/outfits')
      .then((res) => res.json())
      .then(setOutfits)
